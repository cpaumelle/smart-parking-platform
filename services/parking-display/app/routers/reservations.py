...
async def create_reservation(
    ...
    """Create new parking reservation with idempotency support (tenant-scoped)"""
    
    # Check idempotency key for duplicate requests
    if idempotency_key:
        cached = await get_cached_response(idempotency_key)
        if cached:
            logger.info(f"Returning cached response for idempotency key: {idempotency_key} tenant={auth.tenant_slug}")
            return cached
    
    try:
        async with get_tenant_db(auth.tenant_id) as db:
            # Verify space exists (RLS will auto-filter by tenant)
            ...
            logger.info(f"Creating {'immediate' if is_immediate else 'future'} reservation (starts: {reserved_from_utc.isoformat()}, status: {initial_status}) tenant={auth.tenant_slug}")

            # Insert reservation with smart status (tenant_id auto-set via DEFAULT)
            query = """
            ...
            logger.info(f"Created reservation {reservation_id} for space {request.space_id} (status: {initial_status}) tenant={auth.tenant_slug}")
            ...
            if is_immediate:
                ...
                await trigger_space_reconciliation(request.space_id, auth.tenant_id)
            else:
        ...
        logger.error(f"Error creating reservation (tenant={auth.tenant_slug}): {e}", exc_info=True)
...
async def list_reservations(
    ...
    try:
        async with get_tenant_db(auth.tenant_id) as db:
            ...
            logger.info(f"List reservations: tenant={auth.tenant_slug} status={status} count={len(reservations)}")
            return {"reservations": reservations, "count": len(reservations)}

    except Exception as e:
        logger.error(f"Error listing reservations (tenant={auth.tenant_slug}): {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
...
async def cancel_reservation(
    ...
    """Cancel a reservation (tenant-scoped)"""
    try:
        async with get_tenant_db(auth.tenant_id) as db:
            # Get space_id before cancelling (needed for reconciliation trigger)
            # RLS will automatically filter - if not found, could be different tenant
            ...
            # Update reservation status (RLS scoped)
            ...
            logger.info(f"Cancelled reservation {reservation_id} tenant={auth.tenant_slug}")
            ...
            await trigger_space_reconciliation(str(space_id), auth.tenant_id)
        ...
        logger.error(f"Error cancelling reservation (tenant={auth.tenant_slug}): {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
