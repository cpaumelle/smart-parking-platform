...
async def update_space(space_id: str, request: UpdateSpaceRequest, db = Depends(get_db_dependency)):
    ...
    try:
        ...
        logger.info(f"Updated parking space: {result['space_name']} ({space_id})")
...
async def delete_space(
    ...
    """Soft delete parking space (set enabled=FALSE)"""
    try:
        # Check space exists
        existing = await db.fetchrow(
        ...
        if force and active_reservations > 0:
            await db.execute("""
                ...
                WHERE space_id = $1 AND status = 'active'
            """, space_id)
            logger.info(f"Cancelled {active_reservations} reservation(s) for space {space_id}")
        ...
        logger.info(f"Deleted (disabled) parking space: {existing['space_name']} ({space_id})")
